...
public:
    ...
    bool OnGossipHello(Player* player, Creature* me)
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("1v1 Deaktiviert!");
            return true;
        }

        if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Warteschlange verlassen 1v1 Arena", GOSSIP_SENDER_MAIN, 3, "Bist du sicher?", 0, false);
        else
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Warteschlange betreten 1v1 Arena (ungewertet)", GOSSIP_SENDER_MAIN, 20);

        if (player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Erstelle neues 1v1 Arenateam", GOSSIP_SENDER_MAIN, 1, "Bist du sicher?", sConfigMgr->GetIntDefault("CONFIG_ARENA_1V1_COSTS", 400000), false);
        else
        {
            if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Warteschlange betreten 1v1 Arena (gewertet)", GOSSIP_SENDER_MAIN, 2);
                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Arenateam loeschen", GOSSIP_SENDER_MAIN, 5, "Bist du sicher?", 0, false);
            }
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Zeigt deine Statistik", GOSSIP_SENDER_MAIN, 4);
        }
    ...
    bool OnGossipSelect(Player* player, Creature* me, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        case 1: // Create new Arenateam
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Du musst Level %u+ sein um ein 1v1 Arenateam zu erstellen.", sConfigMgr->GetIntDefault("CONFIG_ARENA_1V1_MIN_LEVEL", 80));
        ...
        case 2: // Join Queue Arena (rated)
        {
            if (Arena1v1CheckTalents(player) && JoinQueueArena(player, me, true) == false)
                ChatHandler(player->GetSession()).SendSysMessage("Irgendwas ging falsch beim joinen der Warteschlange.");
        ...
        case 5: // Disband arenateam
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("Arenateam geloescht!");
            player->CLOSE_GOSSIP_MENU();
            return true;
        }
        break;
